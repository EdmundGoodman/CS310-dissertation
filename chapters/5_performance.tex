...
As discussed in \Cref{ch:translation}, a key aspect of the design of the Rust language was preventing undefined behaviour at compile time, through the ownership model enforced by the borrow checker. However, there is not enough information at compile time to fully guarantee these characteristics, for example indexing items in a vector of length known only at runtime. To avoid undefined behaviour, Rust checks the bounds of every indexing operations to guarantee that the index is not out of range. However, this incurs a performance cost -- roughly doubling the time taken for vector indexing. This performance cost can be seen through comparing \texttt{perf} reports when using the default bounds checked indexing in Figure \ref{fig:perf-checked}, against unchecked indexing in Figure \ref{fig:perf-unchecked}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/5_performance/perf/perf_checked_op.png}
    \caption{Screenshot of the \text{perf} report of a Rust translation of \acrshort{HPCCG} using the default checked indexing.}
...
\begin{figure}[H]
    ...
    \caption{Screenshot of the \text{perf} report of a Rust translation of \acrshort{HPCCG} using the unchecked indexing.}
...
% For Kudu and Avon, Slurm is used to dispatch jobs, so the hardware specifications are characterised within a Slurm job to be representative of the compute nodes rather than the login nodes of the cluster.
...
\begin{table}[H]
    ...
    %TC:ignore
    \begin{tabular}{|p{0.13\textwidth}||p{0.65\textwidth}|p{0.12\textwidth}|}
    \hline
...
Scalability can be defined as ``the ability of a system [...] to process growing volumes of work gracefully'' \cite{bondiCharacteristicsScalabilityTheir2000}. This section characterises the strong and weak scaling trends of the Rust and C++ implementations of the \acrshort{HPCCG} \acrshort{mini-app}.
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/scaling/strong_scaling_threaded.tex}}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/scaling/strong_scaling_mpi.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/scaling/strong_scaling_speedup_threaded.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/scaling/strong_scaling_speedup_mpi.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/scaling/weak_scaling_threaded.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/scaling/weak_scaling_mpi.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/scaling/weak_scaling_speedup_threaded.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/scaling/weak_scaling_speedup_mpi.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/parallelism/1_serial_line.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/parallelism/1_serial_line_avon.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/parallelism/2_serial_line_relative.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/parallelism/2_serial_line_relative_avon.tex}}
    \captionsetup{skip=0pt}
...
Having evaluated serial execution, the next parallelisation approach to consider is shared memory parallelism through multi-threading. This is typically used for parallelism within a single machine, but it can also be used in combination with MPI for nodes in a cluster.
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/parallelism/5_parallel_line.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/parallelism/7_parallel_line_relative.tex}}
    \captionsetup{skip=0pt}
...
For the original MPI implementation, the largest mesh size that can be computed without error is constrained by the constant value \mintinline[breaklines,breakafter=\_]{c++}{const int max_external = 100000}, which places an upper bound on the number of cells which can be transmitted between MPI ranks. As a result of this, the largest mesh size computable with two MPI ranks is limited to around $250 \times 250 \times 250$. In order to exercise the MPI implementation across more ranks and larger mesh sizes, the original \acrshort{HPCCG} codebase was modified to set \mintinline{c++}{max_external} to the largest value encodable by a four byte integer, $2147483647$. This allowed very large experiments on meshes up to $800 \times 800 \times 800$ across 5 MPI ranks.

To assess Rust's performance using MPI for message passing, we ran the C++ and Rust executables over both a range of mesh sizes, from $50 \times 50 \times 50$ to $400 \times 400 \times 400$, incrementing each axis size first by $50$, then by $100$ each time. Two nodes and two MPI tasks per node were used in addition to the experimental methodology discussed in section \ref{ssec:experimental-methodology}. The results of these experiments are shown in Figures \ref{fig:9_mpi_line} and \ref{fig:10_mpi_line_relative}.


\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/parallelism/9_mpi_line.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/parallelism/10_mpi_line_relative.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/parallelism/16_kokkos_line.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/parallelism/17_kokkos_line_relative.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/parallelism/25_translation_parallelism_rust.tex}}
    \captionsetup{skip=0pt}
...
\begin{figure}[H]
    \centering
    \fbox{\input{images/5_performance/parallelism/24_translation_parallelism_cpp.tex}}
    \captionsetup{skip=0pt}
